...
```bash
$LIVE amend-signoff <self_id> <message>
```
...
1. **Read the current context first** — run `$LIVE psyche-download <self_id>` and compare what Psyche knows against what happened since signoff. Only amend with information that is missing or different.
