...
2. **Activate the adapter.** Run `spt adapter list` and find `claude-spt`:
   ...
   - Missing or `deregistered` → activate it:
     ...
     - **End-user (plugin only):** `spt adapter add --release BigscreenVR/claude-spt-bs` — fetches the
       ...
       `--tag <ver>` pins a version. Recommended (from the monorepo, no dedicated repo); needs spt
